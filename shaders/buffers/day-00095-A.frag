...
vec4 Field(vec2 p)
{
    ...
    vec2 p0 = clamp(p - vf, vec2(10.), iResolution.xy - 10.0);
    return V_FIELD(p0);
...
void mainImage( out vec4 O, in vec2 I )
{
    ...
    Force -= 0.525*( s.y * -abs(s.x));
    float strength = 3.19;
    Force.x += n.y * strength;  // perpendicular to gradient
    Force.y -= n.x * strength * 1.0;
    ...
    O.xy += (Force) * (sin(T * 0.5) * 0.01 + 0.025 * cos(T * 1.0) * 0.5 + 0.025);
