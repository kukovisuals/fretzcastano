...
void mainImage( out vec4 O, in vec2 I )
{
    ...
    float alpha = T * 0.0131;
    vec2 gradient;
    float n = noise(I * 0.16 + vec2(T * 0.161), period, alpha, gradient);

    // Sample neighboring noise values to create a potential field
    vec2 gradStep = vec2(2.0,2.0); // sampling distance
    vec2 grad2;
    float n_right = noise(I * 0.052516 - vec2(T * 0.61) + vec2(gradStep.x, 0.0), period, alpha, grad2);
    float n_up = noise(I * 0.0062516 + vec2(T * 0.161) + vec2(0.0, gradStep.y), period, alpha, grad2);

    // Create force that flows away from dark areas (toward bright areas)
    vec2 potentialForce;
    potentialForce.x = (n_right - n) / gradStep.x; // flows toward brighter areas
    potentialForce.y = (n_up - n) / gradStep.y;

    // Combine with curl for interesting flow patterns
    vec2 curl = vec2(-gradient.y, gradient.x);
    O.xy += (curl) * 0.005 + potentialForce * 0.02;
